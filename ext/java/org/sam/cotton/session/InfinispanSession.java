...
/* ------------------------------------------------------------ */
public class InfinispanSession extends AbstractSession
{
    private final static Logger __log = Log.getLogger("org.sam.cotton.session");

    private final InfinispanSessionManager _manager;
    ...
    /* ------------------------------------------------------------ */
    public InfinispanSession(InfinispanSessionManager manager, HttpServletRequest request)
    {
    ...
    /* ------------------------------------------------------------ */
    public InfinispanSession(InfinispanSessionManager manager, long created, long accessed, String clusterId, Object version)
    {
    ...
    {
        __log.debug("InfinispanSession:access:active "+_active);
        if (_active.incrementAndGet()==1)
        {
            ...
            else if (period>0)
            {
                ...
                __log.debug("InfinispanSession:access:stale "+stale);
                if (stale>period)
                    refresh();
            }
        }
